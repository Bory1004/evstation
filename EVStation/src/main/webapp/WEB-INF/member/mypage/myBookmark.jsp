...
<div id="center">
	<table style="text-align: center" class="table table-hover caption-top">
		...
		<div id="page">
						<c:if test="${begin > 2}">
							<a href="/myBookmark?p=${begin-1}">[이전]</a>
						</c:if>
						<c:forEach begin="${begin}" end="${end}" var="i">
							<a href="/myBookmark?p=${i}">[${i}]</a>
						</c:forEach>
						<c:if test="${end < totalPage}">
							<a href="/myBookmark?p=${end+1}">[다음]</a>
						</c:if>
		</div>
</div>		
			...
			<div class="btn-group-vertical btn-group-md" role="group" aria-label="Basic example" style="width: 8%;">
				...
				<br>
				<button type="button" class="btn btn-secondary" onclick="location.href='/AllBoardList/${member.memnum}'">내가 쓴 글</button>
				<br>
